...
- 查询全部或单个节点的资源使用率和容量、网络速率、流量配额、负载、运行时长、费用与到期信息。
...
- 卡片包含 CPU 型号与核心、内存/磁盘/交换空间容量、上下行速率、累计流量、流量配额、负载、进程和连接数；面板有提供时还展示费用、到期、GPU 和公开备注。
...
- `image_output` 默认关闭。启用后，AstrBot `html_render` 可能通过你配置的 T2I 服务渲染卡片，节点名、指标、费用、公开备注和 Ping 任务名称会出现在渲染内容中。卡片不主动展示 IP、私有备注或 Token；你自行写进节点名或公开备注的内容仍会展示。对外部服务有隐私顾虑时请保持关闭。
