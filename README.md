...
- 查询全部或单个节点的资源使用率和容量、网络速率、流量配额、负载、运行时长与到期信息。
- 高清 PNG 状态卡片，自动裁掉画布留白；展示电信、联通、移动的延迟、丢包率与样本统计。
...
在插件配置中打开 **“状态和日报使用图片卡片”**（`image_output`），保存后发送 `/kg s` 或 `/kg s 节点名`。`/kg rt` 和定时状态推送也使用相同卡片；渲染失败会自动发送文本。

下面是演示数据生成的单节点卡片，不代表实际服务器：

![单节点高清状态卡片，包含资源容量、流量和三网连接质量](./docs/card-single.png)

单节点占满卡片宽度；多个节点自动分列，奇数末行会铺满，不留下空列。图片高度随内容变化，发送前会裁掉透明画布空白。卡片不含品牌顶栏和页脚，仍保留每个节点的数据来源时间。

- 默认 `image_width=900`、`image_scale=2`，输出宽度为 **1800 像素**的 PNG；清晰度倍率可设为 1–3。
- `image_width` 小于 800 时使用单列。节点较多时可用 `/kg s 节点名` 查看细节，避免聊天软件把长图缩得过小。
- 卡片包含 CPU 型号与核心、内存/磁盘/交换空间容量、上下行速率、累计流量、流量配额、负载、进程和连接数；面板有提供时还展示到期、GPU 和公开备注。卡片不展示费用和账单金额。
- `—` 代表未提供数据。离线节点和历史兜底会标明来源与数据时间；历史 API 不提供的运行时长等信息不会凭空补齐。
- 配额按 Komari 的 `sum/max/min/up/down` 设置计算，优先使用周期流量计数；接口未提供周期计数时使用累计流量，因此可能与面板账期显示有差异。

[查看多节点卡片示例](./docs/card-multiple.png)

### 如何显示三网状态

三网区域读取 **Komari 中已有的 Ping 任务及其记录**，表示“这台服务器 → 指定运营商探测目标”的连接质量，不代表全国该运营商的整体网络状态。插件不会创建探测任务，也不会从 AstrBot 宿主主动测速。

1. 在 Komari 的 Ping 任务管理中，为电信、联通、移动分别配置合适的探测目标，并把任务分配给需要展示的节点。
2. 给任务取明确名称，例如“上海电信”“上海联通”“广州移动”。插件会自动识别运营商；同一节点、同一运营商只有一个匹配任务时直接使用。
3. 如果名称无法识别，或同一运营商存在多个任务，在插件配置的“三网 Ping 任务 ID”中填写要展示的任务 ID；`0` 表示自动识别。ID 是 Komari 任务 ID，不是节点 ID，也不是目标 IP。
4. 等待 Komari 产生记录后发送 `/kg s 节点名`。任务和结果最多缓存 60 秒，刚修改任务时可能需要稍等。

默认统计最近 1 小时，可用 `network_probe_hours` 改为 1–24 小时。延迟显示最近一次探测结果；丢包率为窗口内失败样本数 / 实际样本数，均值和范围只计算成功样本。没有样本不会记为 0% 丢包，也不会把缺报间隔推算为丢包。

| 显示 | 含义及处理 |
| --- | --- |
| 延迟数值 / 丢包率 | 已读取到该任务的实际记录 |
| 超时 | 最近一次失败，或窗口内全部探测失败 |
| 未配置 | 该节点没有匹配任务；检查任务名、ID 和节点分配 |
| 需选任务 | 同运营商匹配到多个任务；在配置中指定一个任务 ID |
| 暂无样本 | 任务存在，但统计窗口内没有记录 |
| 数据过期 | 最近记录超过任务间隔的 3 倍，且至少超过 180 秒 |
| 读取失败 | Ping 接口不可用、无权限或返回无法识别的数据；资源状态仍照常展示 |

三网结果用于状态展示，不新增三网丢包告警。若不需要读取 Ping 数据，可关闭 `network_probe_enabled`。
...
| `image_width` | `900` | 布局宽度，500–1600；低于 800 使用单列 |
| `image_scale` | `2` | 清晰度倍率，1–3；实际图片宽度 = 布局宽度 × 倍率 |
| `network_probe_enabled` | `true` | 在状态报告中读取并展示三网 Ping 结果 |
| `network_probe_hours` | `1` | 延迟和丢包的统计窗口，1–24 小时 |
| `network_probe_tasks` | 各项 `0` | 电信/联通/移动任务 ID；`0` 自动识别 |
...
- `image_output` 默认关闭。启用后，AstrBot `html_render` 可能通过你配置的 T2I 服务渲染卡片，节点名、指标、公开备注和 Ping 任务名称会出现在渲染内容中。卡片不主动展示 IP、私有备注或 Token；你自行写进节点名或公开备注的内容仍会展示。对外部服务有隐私顾虑时请保持关闭。
...
开发者可额外安装 `playwright` 并执行 `playwright install chromium`，再运行 `python scripts/render_preview.py` 重建演示图；脚本检查单节点、多节点、缺失数据、窄卡片和长名称的布局及裁切边界。Playwright 只用于开发预览，不是插件运行依赖。
